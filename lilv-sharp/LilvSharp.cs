	...
	*/
	...
	public class URIDFeature : Lv2FeatureData
	{
		...
		internal class URIDMapMarshal
		{
			...
			public IntPtr MapFunction;
		}
		
		public URIDFeature (Map map)
		{
			_map = (handle, uri) => map (handle, Marshal.PtrToStringAnsi (uri));
			delegate_pin = GCHandle.Alloc (_map, GCHandleType.Pinned);
			var mapptr = Marshal.GetFunctionPointerForDelegate (_map);

			// FIXME: find out why the autogenerated delegate doesn't work.
			/*
			...
			*/
			...
			Pin (new URIDMapMarshal { Handle = new IntPtr (GetHashCode ()), MapFunction = mapptr });
		}
